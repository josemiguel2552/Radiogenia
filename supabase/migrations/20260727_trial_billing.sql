-- Card-first trial billing: the free plan becomes a 7-day trial (card
-- required) that converts into Starter unless canceled. These columns
-- track trial state for the day-6 charge-reminder email and to prevent
...
comment on column public.profiles.trial_used_at is 'Set when the user starts their (only) 7-day trial';
comment on column public.profiles.trial_ends_at is 'When the current/last trial converts to a paid Starter charge';
comment on column public.profiles.trial_reminder_sent_at is 'Day-6 charge reminder email sent (once per trial)';
