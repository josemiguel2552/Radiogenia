...
export default function SupportPage() {
  ...
  const locale = LOCALE_MAP[prefs.uiLanguage || "es"] || "es-ES";
  ...
  const categoryLabel: Record<string, string> = {
    error: t("support.cat_error"),
    question: t("support.cat_question"),
    complaint: t("support.cat_complaint"),
    general: t("support.cat_general"),
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      <header className="sticky top-0 z-20 bg-white/80 dark:bg-gray-950/80 backdrop-blur border-b border-gray-200 dark:border-gray-800">
        <div className="max-w-2xl mx-auto px-4 h-12 flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-gray-900 dark:text-white">{t("support.title")}</span>
        </div>
      </header>

      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">{t("support.my_tickets")}</h2>
            ...
            <CardContent className="p-4 space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={category} onValueChange={(v) => setCategory(v as SupportTicketCategory)}>
                  ...
                  <SelectContent>
                    <SelectItem value="error">{t("support.cat_error_bug")}</SelectItem>
                    <SelectItem value="question">{t("support.cat_question")}</SelectItem>
                    <SelectItem value="complaint">{t("support.cat_complaint")}</SelectItem>
                    <SelectItem value="general">{t("support.cat_general")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs">{t("support.description")}</Label>
                <textarea
                  value={body}
              ...
              <div className="flex gap-2 justify-end">
                ...
                <Button size="sm" onClick={handleSubmit} disabled={sending || !body.trim()} className="gap-1.5">
                  {sending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <><Send className="h-3.5 w-3.5" />{t("support.send")}</>}
                </Button>
              </div>
            </CardContent>
          </Card>
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-sm text-gray-500">{t("support.empty_title")}</p>
              <p className="text-xs text-gray-400 mt-1">{t("support.empty_subtitle")}</p>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-xs font-semibold text-gray-800 dark:text-gray-200 truncate">
                          {ticket.subject || ticket.body.slice(0, 60)}
                        </span>
                        <Badge variant="secondary" className="text-[9px]">{categoryLabel[ticket.category] || ticket.category}</Badge>
                        <Badge variant="secondary" className="text-[9px]">{statusLabel[ticket.status] || ticket.status}</Badge>
                      </div>
                      <p className="text-xs text-gray-600 dark:text-gray-400 whitespace-pre-wrap line-clamp-3">{ticket.body}</p>
                      <span className="text-[10px] text-gray-400 mt-1 block">
                        {new Date(ticket.created_at).toLocaleDateString(locale, { day: "numeric", month: "short", year: "numeric" })}
                      </span>

                      {ticket.admin_reply && (
                        <div className="mt-2 p-2.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-900/40">
                          <span className="text-[10px] font-semibold text-blue-600 dark:text-blue-400 block mb-0.5">{t("support.team_reply")}</span>
                          <p className="text-xs text-blue-800 dark:text-blue-200 whitespace-pre-wrap">{ticket.admin_reply}</p>
                        </div>
                      )}
                    </div>
                  </div>
